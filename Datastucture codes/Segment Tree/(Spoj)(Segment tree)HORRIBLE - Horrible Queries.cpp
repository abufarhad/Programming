...
void   update(ll node, ll c, ll n, ll i, ll j, ll newvalue)
{
    if(i>n|| i<c)
    {
        return;
    ...
    ll right=node*2+1;
    ll mid=(c+n)/2;
    update(left, c, mid, i,j,newvalue);
    update(right, mid+1, n, i,j,newvalue);
    tree[node].sum=tree[left].sum+tree[right].sum+ (n-c+1)*tree[node].prop;
}
...
/*
int  quary(ll node, ll c, ll n, ll i, ll j, ll carry=0 )
{
    if(i>n|| j<c)
    {
        return 0;
